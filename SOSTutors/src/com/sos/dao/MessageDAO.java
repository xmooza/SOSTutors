...
public class MessageDAO {
	...
	public List<Message> list() throws SQLException {
		...
		try {
			pst = connectionManager.getConnection().prepareStatement("select NotificationID, subject, content, date_posted, tutors_tutorID, students_studentID, sessions_sessionID from notifications");
			rs = pst.executeQuery();
		} finally {
			while(rs.next()){
	...
	public Message getMessageFromDB(int MessageId){
		...
		try {
			pst = connectionManager.getConnection().prepareStatement("select * from notifications where NotificationID=?");
			...
			m = new Message(rs.getInt("NotificationID"), rs.getString("subject"), rs.getString("content"),rs.getDate("date_posted"), rs.getInt("tutors_tutorID"), rs.getInt("students_studentID"),rs.getInt("sessions_sessionID"));
	...
	public List<Message> getMessageByRoleAndID(int id, String role) {          
		...
		try { 
			...
			pst = connectionManager.getConnection().prepareStatement("select * from notifications n"
					+ " where n.student_studentID = ?"
					+ " order by c.date_posted desc");
			}
			else
			{
				pst = connectionManager.getConnection().prepareStatement("select * from notifications n"
						+ " where n.tutors_tutorID = ?"
						+ " order by c.date_posted desc");
